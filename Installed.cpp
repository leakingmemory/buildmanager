...
std::vector<InstalledFile> Installed::GetFiles(const std::string &files) {
...
FileMatch Installed::Verify(const std::filesystem::path &rootPath, const InstalledFile &file) {
    ...
    if (exists(path) || is_symlink(path)) {
        ...
        if (hashValue == file.hashOrType) {
            ...
            return FileMatch::NOT_MATCHING;
        }
    } else {
        return FileMatch::NOT_FOUND;
    }
}
...
void Installed::Verify(const std::filesystem::path &rootPath, const std::function<void(const std::filesystem::path &, FileMatch)> &func) const {
    ...
    for (const auto &file : files) {
        std::filesystem::path filePath{file.filename};
        func(filePath, Verify(rootPath, file));
    }
}
